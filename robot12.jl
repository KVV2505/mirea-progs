...
function putmarkers!(r::Robot,n::Int,side::HorizonSide)
    ...
    end # если n=2, то в 0 на правду ставится , если 2, то на ложь
...
if ((isborder(r,side)==false) && (FLAGy==false))
    move!(r,side) #когда начнется с пропуска и потом закрашивается
   #то когда закрашивает, то остается в последней клетки, которую закрасил
end  # и идти начинает с нее, а нужно начинать со следующей
end  #такой проблемы нет когда сначала красится и потом пустота ибо 
  #там делается шаг после закрашивания
